...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($validation !== true) {
            ...
            // Check if email already exists in tbl_admin
            ...
            if ($result->num_rows === 1) {
                $error = '<label class="form-label" style="color:rgb(255, 62, 62);text-align:center;">Already have an admin account for this Email account.</label>';
            } else {
                // Check if email exists in tbl_webuser
                $check_webuser = $database->prepare("SELECT * FROM tbl_webuser WHERE email = ?");
                $check_webuser->bind_param("s", $email);
                $check_webuser->execute();
                $webuser_result = $check_webuser->get_result();
                
                if ($webuser_result->num_rows > 0) {
                    $error = '<label class="form-label" style="color:rgb(255, 62, 62);text-align:center;">This email is already registered as a user. Please use a different email.</label>';
                } else {
                    ...
                    try {
                        ...
                        $insert_admin = $database->prepare("INSERT INTO tbl_admin (email, password) VALUES (?, ?)");
                        $insert_admin->bind_param("ss", $email, $hashed_password);
                        $insert_admin->execute();
                        
                        // Insert into tbl_webuser
                        $insert_webuser = $database->prepare("INSERT INTO tbl_webuser (email, usertype) VALUES (?, 'a')");
                        ...
                        $database->rollback();
                        $error = '<label class="form-label" style="color:rgb(255, 62, 62);text-align:center;">Failed to create admin account. Please try again.</label>';
                    }
                }
            }
        }
    ...
    <div class="container">
        <form action="" method="POST" autocomplete="off" id="signupForm">
            ...
            <table border="0" style="width: 69%;">
                <tr>
                    <td colspan="2">
                        <p class="header-text">Admin Sign Up</p>
                        <p class="sub-text">Create Admin Account</p>
                    </td>
                </tr>
                <tr>
